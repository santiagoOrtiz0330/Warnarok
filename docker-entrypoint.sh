...
LOGIN_CHAR_SERVER_IP=${RATHENA_CHAR_SERVER_IP:-${RAILWAY_PUBLIC:-127.0.0.1}}
LOGIN_CHAR_SERVER_PORT=${RATHENA_CHAR_SERVER_PORT:-6121}
LOGIN_MAP_SERVER_IP=${RATHENA_MAP_SERVER_IP:-${RAILWAY_PUBLIC:-127.0.0.1}}
LOGIN_MAP_SERVER_PORT=${RATHENA_MAP_SERVER_PORT:-5121}
...
# Char server config
# Note: char_port is used for BOTH listening and advertising in rAthena
# We set it to the TCP proxy's external port, and the server will listen on this port
# Railway TCP proxy must be configured to forward: external_port -> same_port (not different)
...
# Map server config
# Note: map_port is used for BOTH listening and advertising in rAthena
# We set it to the TCP proxy's internal port for listening
# Railway TCP proxy must be configured to forward: external_port -> internal_port
